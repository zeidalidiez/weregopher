...
fn exact_artifact_bytes_are_verified() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let verified = verify_transform_artifacts(&overlay, &artifacts, limits)?;
...
fn every_artifact_kind_is_bounded_before_hashing() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for (kind, actual_bytes, limits) in cases {
        let Err(error) = verify_transform_artifacts(&overlay, &artifacts, limits) else {
...
fn overlay(
    ...
    source_map: &[u8],
    audit_log: &[u8],
) -> Result<GeneratedTransformOverlay, Box<dyn std::error::Error>> {
