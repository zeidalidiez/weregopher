...
| Certification evidence | Bounded format-v1 evidence binds the exact compatibility analysis, execution contract/resolution, artifact source, executable, and profile identities to thirteen fixed checks plus at most 128 workflows. Resolved checks require immutable evidence; only incomplete/blocked/complete is derived. Trusted profile resolution, certification-class assignment, signing, publication, and the disposable-state probe runner remain pending |
