...
The canonical bounded parser rejects serialized documents larger than 4 MiB before deserialization. The public evidence domain type deliberately does not implement `Deserialize`, so generic Serde entry points cannot bypass that pre-allocation ceiling. Unknown fields, unsupported format versions, duplicate workflow identifiers, duplicate evidence references, invalid identifiers, contradictory status/evidence combinations, and collection overflows fail closed. The generated schema mirrors structural limits and status/evidence conditions, but Rust remains authoritative.

Canonical compact JSON bytes produce a role-specific `CertificationEvidenceDigest`. Format `"1"` fixes top-level member order as `format_version`, `target`, `profile_digest`, `checks`, `workflows`; target, assessment, and artifact-reference member order is likewise fixed by the contract; fixed checks retain declared dimension order; workflow keys are lexicographically ordered; and evidence references use declared artifact-kind order followed by digest order. The encoding is compact UTF-8 with no BOM, insignificant whitespace, or trailing newline. Checked-in golden bytes and SHA-256 vectors freeze this representation independently of serializer changes. Equivalent insertion orders therefore identify the same exact-target evidence document, while any serialized semantic change changes the content identity. Evidence, profile, and artifact digest wrappers are not substitutable in Rust.
...
- Canonical profile definition and structural profile/evidence binding are specified by [ADR 0029](0029-canonical-certification-profiles.md); [ADR 0031](0031-generation-aware-local-certification-policy.md) adds an explicit generation-aware local approval and class-assignment boundary.
- The concrete disposable-state certification runner, artifact persistence, profile registry, signatures, durable policy persistence, and publication workflow remain future work.
