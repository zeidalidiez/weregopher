...
| OpenAI app-server proxy prerequisite | A platform-neutral transparent proxy core preserves exact valid JSON bytes and unknown protocol fields while enforcing recursive JSON, method/identity, FIFO queue, byte, active-request, history, deadline, and lifecycle bounds. A portable process session attaches to an already-launched piped child, owns bounded stdin/stdout/stderr/reaper workers, enforces initialize-response delivery continuity, classifies crashes and unexpected exits, and performs deadline-bounded graceful/forced shutdown with explicit pipe-drain and abandoned-write accounting. Repository fixtures run on Ubuntu and hosted Windows. This does not authorize or launch the exact bundled app-server, prove Job/process-tree ownership, select a pinned build, mediate approvals/helpers, exercise Codex workflows, or complete G2/G3 |
