...
| OpenAI G2 target feasibility | Canonical package, preload/bridge, app-server, and aggregate evidence contracts; read-only analysis for the maintained exact Windows x64 MSIX identity; a digest-revalidating exact package-derived preload runner; bounded exact-binary schema/initialization probing; and hosted-Windows synthetic isolated-world coverage are implemented. `weregopher feasibility open-ai` can produce all three exact lanes, serially and with explicit acknowledgement, in a user-controlled disposable Windows test account/VM. Public native CI covers repository-only mechanisms; G2 remains operationally incomplete until the final Windows 10/11 exact matrix passes for one pinned build. Configured-preload resolution, vendor renderer/IPC compatibility, certification, security posture, and efficiency remain separate claims |
| OpenAI app-server proxy prerequisite | A platform-neutral transparent proxy core preserves exact valid JSON bytes and unknown protocol fields while enforcing recursive JSON, method/identity, FIFO queue, byte, active-request, history, deadline, and lifecycle bounds. A portable process session attaches to an already-launched piped child, owns bounded stdin/stdout/stderr/reaper workers, enforces initialize-response delivery continuity, classifies crashes and unexpected exits, and performs deadline-bounded shutdown. Prepared frames now provide rollback-free pre-admission checks; a bounded pull journal emits payload/method/wire-ID-redacted lifecycle events with explicit eviction; exact deny-only rules can reduce but never expand traffic authority; and a separate bounded registry correlates explicit thread/turn/item evidence to non-authorizing helper/MCP/command/worktree/browser labels. Repository fixtures run on Ubuntu and hosted Windows. This does not authorize or launch the exact bundled app-server, prove Job/process-tree ownership, derive semantics from unknown payloads, mediate approvals or cleanup effects, select a pinned build, exercise Codex workflows, or complete G2/G3 |
...
crates/
  ...
  weregopher-adapter-openai/  OpenAI G2 evidence and bounded app-server session groundwork
