...
| Certification evidence | Bounded format-v1 evidence binds the exact compatibility analysis, execution contract/resolution, artifact source, executable, and profile identities to thirteen fixed checks plus at most 128 workflows. Canonical format-v1 profiles content-address the declared class, exact fixed-check expectations, and mandatory workflow set; consuming structural validation rejects digest, status, or workflow-scope drift. Exact referenced artifact bytes then require complete bounded kind-and-digest coverage under non-disableable 16 MiB per-artifact and 128 MiB aggregate ceilings. These layers assign no trust; trusted profile approval, semantic report validation, target applicability, certification-class assignment, signing, publication, and the disposable-state probe runner remain pending |
