...
| OpenAI G2 target feasibility | Canonical package, preload/bridge, app-server, and aggregate evidence contracts; read-only analysis for the maintained exact Windows x64 MSIX identity; bounded exact-binary schema/initialization probing; and a hosted-Windows synthetic isolated-world fixture are implemented. `weregopher feasibility open-ai` can produce exact package inventory and, with explicit acknowledgement, exact app-server evidence on a user-controlled Windows installation. G2 remains incomplete until an exact package-derived preload runner produces a passing report for the same build; synthetic evidence cannot satisfy that lane, and no application compatibility, certification, security-posture, or efficiency claim follows |
