...
The committed foundation contains the Rust workspace, canonical platform-neutral domain and protocol contracts, deterministic checked-in JSON Schemas, pure package-tree manifest construction from pre-observed records, a bounded Windows primitive that hashes one leased direct file, and read-only candidate profiles plus provenance-bound installation evidence contracts for Codex, Hermes Agent, Discord, and Visual Studio Code. Bounded Windows discovery checks maintained per-user Discord Stable/PTB/Canary and Visual Studio Code Stable/Insiders locations and read-only HKCU/HKLM 32-bit and 64-bit uninstall-registry views. Registry matches require an exact maintained product/publisher pair, an absolute installation root, and direct marker-file metadata; neither source recurses or infers compatibility. Additional Codex and Hermes Agent sources, package-catalog sources, package-root traversal, coherent package scanning, and executable runtime components remain separate follow-up increments.
