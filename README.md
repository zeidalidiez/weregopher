...
| Certification evidence | Bounded canonical profiles, evidence, exact artifact-byte verification, generation-aware local class assignment, atomic local-only receipts, runner identity, and runner policy are implemented. The first concrete format-v1 Discord report validates one exact adapter transform and disposable-state marker run, derives a smoke-only compatibility/profile/evidence bundle, and requires a repeatable operator-pinned report before assigning `smoke_verified`; component authentication, runner/report attestation, freshness, registry signatures, durable policy/receipt persistence, and the general scenario runner remain pending, and no certification result authorizes transformation or execution |
