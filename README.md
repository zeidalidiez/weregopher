...
| Certification evidence | Bounded canonical profiles, evidence, exact artifact-byte verification, generation-aware local class assignment, runner identity/policy, exact component-descriptor and artifact verification, single-use monotonic freshness, atomic dual-policy local attestations, and a bounded hash-chained filesystem ledger accepted only under an independently pinned head are implemented. The Discord marker flow loads a closed runner bundle, launches a retained package snapshot, binds the exact semantic report and both policies, and records a local-only attestation; authenticated bundle distribution, remote signatures/key lifecycle, external revocation, the general scenario runner, and full compatibility/security/efficiency certification remain pending, and no certification result authorizes transformation or execution |
...
single-use pre-run challenge, and a rollback-pinned durable ledger. It remains an
explicitly uncertified diagnostic launch of the copied vendor Electron runtime.
