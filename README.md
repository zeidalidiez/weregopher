...
| OpenAI app-server proxy prerequisite | A platform-neutral transparent proxy core preserves exact valid JSON bytes and unknown protocol fields while enforcing recursive JSON, method/identity, FIFO queue, byte, active-request, history, deadline, and lifecycle bounds. It correlates independent bidirectional request spaces, distinguishes late and unmatched responses, rejects ambiguous wire-ID reuse, and exposes payload-redacted diagnostics. It does not launch or connect to the bundled app-server, select a pinned build, mediate approvals/helpers, exercise Codex workflows, or complete G2/G3 |
...
crates/
  ...
  weregopher-adapter-openai/  OpenAI G2 evidence and app-server proxy groundwork
